...
def overlap(year, category):
    ...
    return(year, len(p_titles) - len(both_titles), len(both_titles), len(n_titles) - len(both_titles), overlap_winner(p_sample, n_sample))

def overlap_winner(p_sample, n_sample):
    p_sample = [e for e in p_sample if e['outcome'] == 'winner']
    n_sample = [e for e in n_sample if e['outcome'] == 'winner']
    p_titles = set([e['title'] for e in p_sample])
    n_titles = set([e['title'] for e in n_sample])
    both_titles = p_titles.intersection(n_titles)
    if len(both_titles) == 1:
        return 1
    if len(both_titles) == 0:
        return 0
    else:
        print('DEBUG ME! more than 1 winner:', both_titles)
...
def overlap_category(category):
    ...
    with open(filename, 'w', newline='') as output:
        ...
        writer.writerow(['year', '# finalist for pulitzer only', '# finalist for both', '# finalist for nba only', 'shared winner'])
